...
const BASE = "/admin/wellness-team-notes";

function tokenOrStored(token) {
  return token || getAccountToken();
}

export function mapWellnessTeamNote(row) {
  if (!row) return null;
...
export async function adminListWellnessTeamNotes(token, { page = 1, limit = 20, status, search, designation } = {}) {
  const params = { page, limit };
  if (status) params.status = status;
  if (String(search || "").trim()) params.search = String(search).trim();
  if (String(designation || "").trim()) params.designation = String(designation).trim();
  try {
    const { data } = await api.get(BASE, {
      params,
      headers: authHeader(tokenOrStored(token)),
    });
    const items = (Array.isArray(data.wellnessTeamNotes) ? data.wellnessTeamNotes : [])
      .map(mapWellnessTeamNote)
      .filter(Boolean);
    return {
      items,
...
export async function adminCreateWellnessTeamNote(token, fields, files = {}) {
  ...
  try {
    ...
    return mapWellnessTeamNote(data.wellnessTeamNote);
  } catch (error) {
    normalizeApiError(error);
  }
}

export async function adminUpdateWellnessTeamNote(token, id, fields = {}, files = {}) {
  const headers = authHeader(tokenOrStored(token));
  try {
    const photo = files.profileImage instanceof File ? files.profileImage : files.file instanceof File ? files.file : null;
...
export async function adminDeleteWellnessTeamNote(token, id) {
  try {
