...
import {
  REAL_PEOPLE_CONCERNS,
  REAL_PEOPLE_FIELDS,
  REAL_PEOPLE_GALLERY_OWNERS,
  REAL_PEOPLE_RATINGS,
      ...
      <div className="ua-cfg-mv-upload-modal ua-cfg-pt-upload-modal" onClick={(event) => event.stopPropagation()} role="dialog">
        <div className="ua-cfg-mv-upload-modal__head">
          <div>
            ...
            <p className="ua-cfg-mv-upload-modal__sub">photo · set the crop, ratio and zoom before it is attached</p>
          </div>
...
function PointValue({ entry, onChange }) {
  if (entry.options === "concern") {
    return (
      <select value={entry.value} onChange={(event) => onChange(event.target.value)}>
        {REAL_PEOPLE_CONCERNS.map((option) => (
          <option key={option} value={option}>{option}</option>
        ))}
      </select>
    );
  }
  if (entry.options === "rating") {
    return (
      <select value={entry.value} onChange={(event) => onChange(event.target.value)}>
        {REAL_PEOPLE_RATINGS.map((option) => (
          <option key={option} value={option}>{option}</option>
        ))}
      </select>
    );
...
export function RealPeopleSection({
  ...
  const availableFields = REAL_PEOPLE_FIELDS.filter((entry) => !usedFields.has(entry.id));
  ...
  return (
    <div className="ua-cfg-tf ua-cfg-rp">
      ...
      <div className="ua-cfg-rp-grid">
        <Panel title="Photo">
          <div className={`ua-cfg-tf-drop ua-cfg-tf-drop--after${editor.uploaded ? " is-on" : ""}`}>
            <p>📷</p>
            <span>Client photo</span>
            <button type="button" className="ua-cfg-btn ua-cfg-btn--ghost ua-cfg-btn--sm" onClick={() => setUploadKind("photo")}>
              {editor.uploaded ? "Replace" : "Upload photo"}
            </button>
          </div>
        </Panel>
                ...
                <div className="ua-cfg-tf-add__menu">
                  {availableFields.length ? availableFields.map((entry) => (
                    <button
                      ...
                      type="button"
                      onClick={() => {
                        setPoints((prev) => [...prev, { id: `rp-${Date.now()}`, field: entry.id, label: entry.label, value: entry.sample, source: "AUTO" }]);
                        setAddOpen(false);
          ...
          <div className="ua-cfg-tf-table">
            ...
            {points.map((entry) => (
              <div key={entry.id} className="ua-cfg-tf-table__row">
                ...
                <PointValue
                  entry={entry}
                  onChange={(value) => setPoints((prev) => prev.map((row) => (row.id === entry.id ? { ...row, value, source: entry.source === "REQUIRED" ? "REQUIRED" : "EDIT" } : row)))}
                />
                <em className={entry.source === "REQUIRED" ? "is-required" : ""}>{entry.source}</em>
                {entry.source === "REQUIRED" ? <span /> : (
                  <button
                    type="button"
                ...
                <div className="ua-cfg-bn-preview__web">
                  ...
                  <div className="ua-cfg-bn-preview__browser">
                    <div className="ua-cfg-bn-preview__chrome">
                      ...
                      <strong>Real People Real Healing</strong>
                      <em>irwellness.in</em>
                    </div>
                    <div className={`ua-cfg-bn-preview__banner ua-cfg-tf-preview-image${editor.uploaded ? " is-on" : ""}`}>IMAGE</div>
                  </div>
                </div>
              ) : null}
              {editor.appOn ? (
                <div className="ua-cfg-bn-preview__app">
                  ...
                  <div className="ua-cfg-bn-preview__phone">
                    ...
                    <div className={`ua-cfg-bn-preview__banner ua-cfg-bn-preview__banner--app ua-cfg-tf-preview-image${editor.uploaded ? " is-on" : ""}`}>IMAGE</div>
                  </div>
                </div>
              ) : null}
            </div>
      ...
      >
        <div className="ua-cfg-mv-gallery__filters">
          ...
          <select className="ua-cfg-mv-gallery__select" value={owner} onChange={(event) => setOwner(event.target.value)}>
            {REAL_PEOPLE_GALLERY_OWNERS.map((entry) => (
      ...
      <UploadConfirmModal
        ...
        onConfirm={() => {
          if (uploadKind === "photo") patch({ uploaded: true });
          else {
            setGallery((prev) => [
              {
                id: `rp-g-${Date.now()}`,
                title: "New real people photo",
                owner: "Anita Rao",
                date: "14 Aug 2026",
                size: "1.9 MB",
                versions: 1,
