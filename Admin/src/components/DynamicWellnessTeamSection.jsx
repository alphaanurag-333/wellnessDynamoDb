...
import {
  adminCreateWellnessTeamNote,
  adminDeleteWellnessTeamNote,
  adminListWellnessTeamNotes,
  adminUpdateWellnessTeamNote,
} from "../api/wellnessTeamNoteApi.js";
...
function WellnessTeamViewModal({ entry, onClose, onEdit }) {
      ...
      <div className="ua-cfg-rc-view ua-cfg-ld-view" onClick={(event) => event.stopPropagation()} role="dialog" aria-labelledby="ld-view-title">
        <div className="ua-cfg-rc-view__head">
          <div>
            ...
            <h3 id="ld-view-title">{asCopyString(entry.name) || "Untitled team member"}</h3>
            <p>
        ...
        <div className="ua-cfg-rc-view__foot">
          ...
          <button type="button" className="ua-cfg-btn ua-cfg-btn--primary" onClick={() => { onClose(); onEdit(entry.id); }}>Edit profile</button>
        </div>
      </div>
    </div>
  );
...
export function DynamicWellnessTeamSection({ items, setItems, onToast }) {
  ...
  const loadTitles = useCallback(async () => {
    try {
      const list = await adminGetConfigDropdown(null, "wellness-title");
  ...
  const loadItems = useCallback(async (pageOverride) => {
    ...
    setLoading(true);
    try {
      const result = await adminListWellnessTeamNotes(null, {
        page: nextPage,
        limit: PAGE_SIZE,
      ...
      setItems([]);
      onToast(error?.message || "Could not load wellness team profiles");
    } finally {
      setLoading(false);
    }
  }, [onToast, page, query, statusFilter, designationFilter, setItems]);
  ...
  useEffect(() => {
    setPage(1);
  }, [query, statusFilter, designationFilter]);
  ...
  async function confirmCoverCrop(croppedFile, cropError) {
    ...
    setBusy(true);
    try {
      const saved = await adminUpdateWellnessTeamNote(null, target, {}, { file: croppedFile });
  ...
  async function addItem() {
    ...
    setBusy(true);
    try {
      await adminCreateWellnessTeamNote(null, {
        name,
        designation,
      ...
      setCreating(false);
      setPage(1);
      onToast("Wellness team profile added");
      await loadItems(1);
    } catch (error) {
      onToast(error?.message || "Could not add wellness team profile");
    } finally {
      setBusy(false);
    }
  }
  ...
  async function saveItem(item) {
    ...
    setBusy(true);
    try {
      const saved = await adminUpdateWellnessTeamNote(null, item.id, {
        name,
        designation,
      ...
      setEditingId(null);
      onToast("Wellness team profile saved");
    } catch (error) {
      onToast(error?.message || "Could not save wellness team profile");
    } finally {
      setBusy(false);
    }
  }
  ...
  async function toggleLive(item) {
    ...
    try {
      const saved = await adminUpdateWellnessTeamNote(null, item.id, { live });
      patchItem(item.id, saved);
    } catch (error) {
  ...
  async function toggleSurface(item, field) {
    ...
    try {
      const saved = await adminUpdateWellnessTeamNote(null, item.id, { [field]: next });
      patchItem(item.id, saved);
    } catch (error) {
  ...
  async function deleteItem() {
    ...
    setBusy(true);
    try {
      await adminDeleteWellnessTeamNote(null, item.id);
      onToast("Wellness team profile deleted");
      await loadItems();
    } catch (error) {
      onToast(error?.message || "Could not delete wellness team profile");
    } finally {
      setBusy(false);
    }
  }
  ...
  return (
    <div className="ua-cfg-rc ua-cfg-ld ua-cfg-wt">
      ...
      >
        {creating ? (
          <section className="ua-cfg-rc-new ua-cfg-ld-new">
            <div className="ua-cfg-rc-new__head">
              <strong><span aria-hidden="true">✦</span> New team profile</strong>
            ...
            <NoteForm
              ...
              busy={busy}
              saveLabel={busy ? "Saving…" : "Add profile"}
        ...
        <div className="ua-cfg-rc-toolbar">
          <input
            ...
            aria-label="Search wellness team profiles"
          />
          <CfgSelect
            className="ua-cfg-select--filter"
            options={designationFilters}
        ...
        {items.length ? (
                      ...
                      <div className="ua-cfg-ld-item__actions">
                        <div className="ua-cfg-ld-item__surfaces">
                          ...
                          <div className="ua-cfg-ld-item__live">
                            ...
                            <button
                              ...
                              aria-label={item.live ? "Hide profile" : "Publish profile"}
                              disabled={busy}
                        ...
                        <div className="ua-cfg-ld-item__btns">
                          ...
                          <button
                            type="button"
                            className="ua-cfg-icon-btn"
                            aria-label={`Delete ${asCopyString(item.name) || "profile"}`}
                            disabled={busy}
          ...
          <p className="ua-cfg-panel__sub">
            {loading
              ? "Fetching wellness team profiles…"
              : hasListFilter
                ? "No wellness team profiles match this filter."
                : "No wellness team profiles yet. Add the first coach or nutritionist."}
          </p>
        )}
        ...
        <ListPagination
          ...
          label="Wellness team profiles pagination"
        />
      </Panel>

      <WellnessTeamViewModal
        entry={viewing}
      ...
      <ConfirmDialog
        ...
        tag="Wellness team profile"
        title={`Delete ${asCopyString(pendingDelete?.name) || "this profile"}?`}
        body="This permanently removes the wellness team profile and its portrait."
        confirmLabel="Delete"
        confirmTone="danger"
