...
function lineHeightPx(style) {
  ...
  if (!raw || raw === "normal") return fontSize * 1.45;
  const value = parseFloat(raw);
  if (!Number.isFinite(value) || value <= 0) return fontSize * 1.45;
...
function useInlineTruncate(text, expanded, lines) {
...
export function CfgInlineReadMore({
  text,
  expanded,
  onToggle,
  lines = 2,
  className = "",
  buttonClassName = "ua-cfg-rm-btn",
  empty = null,
}) {
  const source = String(text || "").trim();
  const { ref, overflows, preview } = useInlineTruncate(source, expanded, lines);
  const showToggle = Boolean(source) && (overflows || expanded);
  const visible = expanded || !overflows ? source : preview;

  if (!source) {
    return empty == null ? null : <p className={className}>{empty}</p>;
  }

  return (
    <p ref={ref} className={className}>
      <span>{visible}{showToggle && expanded ? " " : ""}</span>
      {showToggle ? (
        <button
          type="button"
