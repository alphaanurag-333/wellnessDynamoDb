  ...
  return (
    <div className="ua-cfg-ch">
      ...
      <Panel
        ...
        subtitle={`${pagination.total} total · ${liveCount} live on the public feed — edit the message or hide a card.`}
        actions={
          <CfgSelect
      ...
      >
        {items.length ? (
          <div className={`ua-cfg-ch-cards${loading ? " is-loading" : ""}`}>
            {items.map((entry) => {
              const comments = Number(entry.commentCount) || 0;
              const days = entry.daysSubmitted != null ? Number(entry.daysSubmitted) : null;
              return (
                <article
                  key={entry.id}
                  className={`ua-cfg-ch-card${entry.live ? " ua-cfg-ch-card--live" : ""}`}
                >
                  <Avatar src={entry.profileImage} name={asCopyString(entry.name)} />
                  <div className="ua-cfg-ch-card__copy">
                    <div className="ua-cfg-ch-card__meta">
                      <strong>{asCopyString(entry.name)}</strong>
                      {entry.rank != null ? (
                        <span className="ua-cfg-ch-card__rank">Rank #{entry.rank}</span>
                      ) : null}
                      <span className={`ua-cfg-faq__shown${entry.live ? " is-on" : ""}`}>
                        {entry.live ? "LIVE" : "HIDDEN"}
                      </span>
                    </div>
                    <p className="ua-cfg-ch-card__message">
                      {asCopyString(entry.message) || "No message"}
                    </p>
                    <p className="ua-cfg-ch-card__info">
