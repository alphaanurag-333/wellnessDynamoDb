...
async function listAllSectionsUnpaged() {
  ...
  return (items || []).map(toPublicSection).filter(Boolean);
}

async function nextSortOrder() {
  const items = await listAllSectionsUnpaged();
...
async function createDrfSection({
  name,
  weight = 0,
  live = true,
  fixed = false,
  ...
  return toPublicSection(item);
}

async function getDrfSectionById(id) {
  const { Item } = await docClient.send(new GetCommand({ TableName: TABLE, Key: { id } }));
  return toPublicSection(Item || null);
}

async function getDrfSectionRecordById(id) {
  const { Item } = await docClient.send(new GetCommand({ TableName: TABLE, Key: { id } }));
  return withLegacyId(Item || null);
}

async function updateDrfSection(id, updates) {
  ...
  return toPublicSection(Attributes);
}

async function deleteDrfSection(id) {
  await docClient.send(
    new DeleteCommand({
      TableName: TABLE,
      Key: { id },
...
async function listDrfSections({ page = 1, limit = 50, status } = {}) {
  ...
  return {
    sections: items.map(toPublicSection).filter(Boolean),
    pagination,
  };
}
