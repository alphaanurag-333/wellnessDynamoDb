...
function normalizeBpm(value, fieldName) {
  const n = Number.parseInt(String(value ?? ""), 10);
  if (!Number.isFinite(n) || n < 1 || n > 300) {
    const err = new Error(`${fieldName} must be an integer between 1 and 300`);
    err.name = "ValidationError";
    throw err;
  }
  return n;
...
function normalizeSampleCount(value) {
  ...
  if (!Number.isFinite(n) || n < 0 || n > 100000) {
    const err = new Error("sampleCount must be an integer between 0 and 100000");
    err.name = "ValidationError";
    throw err;
  }
  return n;
...
function normalizeOptionalIsoTime(value) {
  ...
  if (Number.isNaN(date.getTime())) {
    const err = new Error("latestRecordedAt must be a valid ISO timestamp");
    err.name = "ValidationError";
    throw err;
  }
...
function formatDayLog(item) {
  ...
  return {
    date,
    day: dayLabel(date),
    latestBpm: Number(item?.latestBpm ?? 0),
    latestRecordedAt: item?.latestRecordedAt ?? null,
    restingBpm: Number(item?.restingBpm ?? 0),
    averageBpm: Number(item?.averageBpm ?? 0),
    maxBpm: Number(item?.maxBpm ?? 0),
...
async function upsertDayLog(userId, date, payload) {
  const now = new Date().toISOString();
  const latestBpm = normalizeBpm(payload.latestBpm, "latestBpm");
  const restingBpm = normalizeBpm(payload.restingBpm, "restingBpm");
  const averageBpm = normalizeBpm(payload.averageBpm, "averageBpm");
  const maxBpm = normalizeBpm(payload.maxBpm, "maxBpm");
  const sampleCount = normalizeSampleCount(payload.sampleCount);
  const latestRecordedAt = normalizeOptionalIsoTime(payload.latestRecordedAt);
  ...
  const { Attributes } = await docClient.send(
    new UpdateCommand({
      ...
      Key: { userId, recordKey: dayRecordKey(date) },
      UpdateExpression:
        "SET latestBpm = :latestBpm, latestRecordedAt = :latestRecordedAt, restingBpm = :restingBpm, averageBpm = :averageBpm, maxBpm = :maxBpm, sampleCount = :sampleCount, #date = :date, #source = :source, externalIds = :externalIds, syncedAt = :syncedAt, platform = :platform, dataOrigin = :dataOrigin, updatedAt = :updatedAt, createdAt = if_not_exists(createdAt, :createdAt)",
...
async function syncHeartRateRecords(userId, { platform, source, records }) {
  const normalizedPlatform = normalizePlatform(platform);
  ...
  for (const record of mergedRecords) {
    ...
    const incoming = {
      latestBpm: record.latestBpm,
      latestRecordedAt: record.latestRecordedAt,
      restingBpm: record.restingBpm,
      averageBpm: record.averageBpm,
...
async function getUserHeartRateSummary(userId, { date, days = 7 } = {}) {
