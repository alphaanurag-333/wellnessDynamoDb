...
const { resolveStaffActor, assertStaffCanAccessUser, getStaffScopeCoachId } = require("../staffAccess");
const {
...
function getCoachId(req) {
  ...
  return getStaffScopeCoachId(req) || actor.id;
}

async function loadAccessibleUser(req, userId) {
...
async function assertCanAccessUserProgram(req, program) {
  if (!program) throw new AppError("Program assignment not found", 404);
  const actor = resolveStaffActor(req);
  if (actor.role === "admin") return actor;
  if (String(program.coachId) !== String(getCoachId(req))) {
...
exports.listProgramsForUserController = asyncHandler(async (req, res) => {
  ...
  await loadAccessibleUser(req, userId);
  const actor = resolveStaffActor(req);
  const result = await listUserProgramsByUserId(userId, { page: 1, limit: 50 });
  const programs =
    actor.role === "admin"
      ? result.items
      : result.items.filter((p) => String(p.coachId) === String(getCoachId(req)));

  return res.status(200).json({
    status: true,
    message: "User program assignments fetched",
    programs: programs.map(toPublicUserProgram),
  });
});
...
exports.createProgramAssignmentController = asyncHandler(async (req, res) => {
  ...
  const user = await loadAccessibleUser(req, userId);
  const actor = resolveStaffActor(req);
  const coachId = actor.role === "wellness_coach" ? actor.id : String(user.parentCoachId || actor.id);
...
exports.updateProgramAssignmentController = asyncHandler(async (req, res) => {
  ...
  await assertCanAccessUserProgram(req, program);
...
exports.getProgramForClientController = asyncHandler(async (req, res) => {
  ...
  const myPrograms =
    actor.role === "admin"
      ...
      : assignmentsResult.items.filter((p) => String(p.coachId) === String(getCoachId(req)));
