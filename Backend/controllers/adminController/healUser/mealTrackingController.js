...
exports.listAdminUserMealTrackingController = asyncHandler(async (req, res) => {
...
exports.createAdminUserMealLogController = asyncHandler(async (req, res) => {
  const adminId = req.auth?.sub;
  if (!adminId) throw new AppError("Unauthorized", 401);
  ...
  try {
    mealLog = await createMealLog({
      ...
      loggedById: adminId,
      assignedCoachId: adminId,
      assignedAdminType: "wellness_coach",
    });
  } catch (err) {
...
exports.updateAdminUserMealLogController = asyncHandler(async (req, res) => {
  const adminId = req.auth?.sub;
  if (!adminId) throw new AppError("Unauthorized", 401);
...
exports.deleteAdminUserMealLogController = asyncHandler(async (req, res) => {
  const adminId = req.auth?.sub;
  if (!adminId) throw new AppError("Unauthorized", 401);
...
exports.updateAdminUserMealTrackingModeController = asyncHandler(async (req, res) => {
  const adminId = req.auth?.sub;
  if (!adminId) throw new AppError("Unauthorized", 401);
