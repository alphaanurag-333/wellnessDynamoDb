...
} = require("../../models/wellnessYogaModel");

const S3_FOLDER = "wellness-yoga";
...
async function uploadWellnessYogaMedia(req) {
  const thumbnail =
...
exports.listWellnessYogaController = asyncHandler(async (req, res) => {
  const { page = 1, limit = 10, status, type, search } = req.query;
  const data = await listWellnessYoga({ page, limit, status, type, search });
  return res.status(200).json({ status: true, items: data.items, pagination: data.pagination });
});

exports.getWellnessYogaByIdController = asyncHandler(async (req, res) => {
  const item = await getWellnessYogaById(req.params.id);
  if (!item) throw new AppError("Yoga item not found", 404);
  return res.status(200).json({ status: true, item });
});

exports.createWellnessYogaController = asyncHandler(async (req, res) => {
  ...
  const { thumbnail: uploadedThumb, file: uploadedFile } = await uploadWellnessYogaMedia(req);
  ...
  if (!WELLNESS_YOGA_ALLOWED_TYPE.includes(type)) {
    throw new AppError("type must be video, audio, or ytlink", 400);
  }
  if (!WELLNESS_YOGA_ALLOWED_STATUS.includes(status)) {
  ...
  const item = await createWellnessYoga({ title, type, ytLink, file, thumbnail, duration, status });

  return res.status(201).json({ status: true, message: "Yoga item created successfully", item });
});

exports.updateWellnessYogaController = asyncHandler(async (req, res) => {
  const current = await getWellnessYogaRecordById(req.params.id);
  if (!current) throw new AppError("Yoga item not found", 404);
  ...
  let item;
  try {
    item = await updateWellnessYoga(req.params.id, updates);
  } catch (err) {
    if (err?.name === "ConditionalCheckFailedException") throw new AppError("Yoga item not found", 404);
    throw err;
  }
  return res.status(200).json({ status: true, message: "Yoga item updated successfully", item });
});

exports.deleteWellnessYogaController = asyncHandler(async (req, res) => {
  const current = await getWellnessYogaRecordById(req.params.id);
  if (!current) throw new AppError("Yoga item not found", 404);
  ...
  try {
    await deleteWellnessYoga(req.params.id);
  } catch (err) {
    if (err?.name === "ConditionalCheckFailedException") throw new AppError("Yoga item not found", 404);
    throw err;
  }
  return res.status(200).json({ status: true, message: "Yoga item deleted successfully" });
});
