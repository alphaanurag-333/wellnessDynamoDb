...
async function assertActivePost(postId) {
  const post = await getMonthlyChampionPostById(postId);
...
exports.listMonthlyChampionCommentsController = asyncHandler(async (req, res) => {
  await assertActivePost(req.params.postId);
  const { page, limit } = readPaging(req.query);
  const data = await listMonthlyChampionPostComments({
...
exports.createMonthlyChampionCommentController = asyncHandler(async (req, res) => {
  ...
  if (post.userId && post.userId !== commenterUserId) {
    ...
    dispatchMonthlyChampionCommentNotification({
      ...
      message: `${commenterName} commented on your Champion post: "${comment}"`,
      comment,
    }).catch((err) => console.error("Monthly champion comment notification failed:", err?.message || err));
  }
...
exports.deleteMonthlyChampionCommentController = asyncHandler(async (req, res) => {
  await assertActivePost(req.params.postId);
  ...
  try {
    await deleteMonthlyChampionPostComment(req.params.id);
  } catch (err) {
