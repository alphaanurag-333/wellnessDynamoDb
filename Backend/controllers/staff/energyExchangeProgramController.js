...
function assertCanAccessProgram(req, program) {
...
exports.getProgramController = asyncHandler(async (req, res) => {
  ...
  await assertCanAccessProgram(req, program);
  return res.status(200).json({
    status: true,
...
exports.getEnergyExchangeForUserController = asyncHandler(async (req, res) => {
  ...
  const myPrograms =
    actor.role === "admin"
      ...
      : programsResult.items.filter((p) => String(p.coachId) === String(getCoachId(req)));
  const appConfig = await getAppConfig();
