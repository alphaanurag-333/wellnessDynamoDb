...
End-to-end consultancy checkout: pricing, Cashfree payment, meeting assignment, Zoom, WhatsApp, PDF invoice, and portal visibility.
...
3. Client completes payment (Cashfree SDK with `paymentSessionId` + `mode`)
4. `POST /api/user/consultancy-payment/verify`
5. `GET /api/user/consultancy-payment/transactions`

Create-order fails with `PaymentGatewayError` when Cashfree keys are not configured in Admin → Configs → Payment gateway.
...
**App config:** `consultancy_amount`, `tax_type`, `tax_value`, `referral_discount`, Cashfree gateway keys (UAT + Live) and mode.
