...
| GET | `/api/public/misc/pages/:slug` | Static CMS page. App legal: `app-privacy-policy`, `app-terms-of-service`, `app-community-guidelines`, `app-dpa`. Use `?platform=app`. Compliance is a drawer line on `GET /api/public/app-config` (`compliance_enabled`, `compliance_names`). |
