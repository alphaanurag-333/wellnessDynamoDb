...
| `GET` | `/public/misc/pages/:slug` | Static / legal / about pages. App legal slugs: `app-privacy-policy`, `app-terms-conditions`, `app-community-guidelines`, `app-dpa`. Shared / web: `privacy-policy`, `terms-and-conditions`, `community-guideline`. Pass `?platform=app` for app-surface block versions. Compliance is **not** a page — use `GET /public/app-config` fields `compliance_enabled` + `compliance_names`. |
