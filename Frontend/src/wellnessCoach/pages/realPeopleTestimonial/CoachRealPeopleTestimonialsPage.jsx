...
import {
  ...
  coachListPendingRealPeopleTestimonials,
...
export function CoachRealPeopleTestimonialsPage() {
  ...
  const loadRows = useCallback(async () => {
    ...
    try {
      if (tab === "pending") {
        const pending = await coachListPendingRealPeopleTestimonials(coachToken);
        setRows(pending);
      } else {
        const { realPeopleTestimonials } = await coachListRealPeopleTestimonials(coachToken, { limit: 50 });
        setRows(realPeopleTestimonials);
      }
    } catch (e) {
      if (e?.status === 401) return dispatch(logoutCoach());
      await Swal.fire({ icon: "error", title: "Load failed", text: e.message });
    } finally {
      setLoading(false);
    }
  }, [coachToken, dispatch, tab]);
  ...
  const onReview = async (row, action) => {
    ...
    try {
      await coachReviewRealPeopleTestimonial(coachToken, row._id, { action });
  ...
  const onToggle = async (row) => {
    ...
    try {
      await coachUpdateRealPeopleTestimonial(coachToken, row._id, { status: nextStatus });
  ...
  const onDelete = async (row) => {
    ...
    try {
      await coachDeleteRealPeopleTestimonial(coachToken, row._id);
      await loadRows();
    } catch (e) {
      if (e?.status === 401) return dispatch(logoutCoach());
