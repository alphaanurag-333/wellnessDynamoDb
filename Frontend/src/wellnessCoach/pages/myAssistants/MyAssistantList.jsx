...
import { LIST_LIMIT, formatPhone, resolveAssistantId } from "./MyAssistantShared.js";

export function MyAssistantList() {
  ...
  const loadRows = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const { assistants, pagination } = await coachListAssistants(coachToken, {
        page,
        limit: LIST_LIMIT,
  ...
  }, [coachToken, debouncedSearch, dispatch, page, statusFilter]);
  ...
  const handleDelete = async (row) => {
    const id = resolveAssistantId(row);
    const { isConfirmed } = await Swal.fire({
      title: "Delete assistant?",
      html: `Remove <strong>${row.name || row.email}</strong>?`,
      icon: "warning",
    ...
    try {
      await coachDeleteAssistant(coachToken, id);
  ...
  const handleToggleStatus = async (row) => {
    ...
    setTogglingId(id);
    try {
      await coachUpdateAssistant(coachToken, id, { status: nextStatus });
                  ...
                  <tr key={id}>
                    ...
                    <td>
                      <button
                        ...
                        title={row.status === "active" ? "Deactivate assistant" : "Activate assistant"}
                      >
