...
function base() {
  return "/coach/real-people-testimonials";
}

export async function coachListRealPeopleTestimonials(token, { page = 1, limit = 20, status, approvalStatus, search } = {}) {
...
export async function coachListPendingRealPeopleTestimonials(token) {
  try {
...
export async function coachReviewRealPeopleTestimonial(token, id, { action, rejectionReason } = {}) {
  try {
...
export async function coachUpdateRealPeopleTestimonial(token, id, fields) {
  const payload = {};
  ...
  try {
    const { data } = await coachApi.patch(`${base()}/${encodeURIComponent(id)}`, payload, { headers: authHeader(token) });
    return data.realPeopleTestimonial;
  } catch (error) {
    normalizeApiError(error);
  }
...
export async function coachDeleteRealPeopleTestimonial(token, id) {
  try {
