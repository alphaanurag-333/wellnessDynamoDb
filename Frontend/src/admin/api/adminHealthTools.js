...
function base() {
  return "/admin/health-tools";
}

export async function adminListHealthTools(token, { page = 1, limit = 50, status, search } = {}) {
  ...
  try {
    ...
    return {
      healthTools: Array.isArray(body.healthTools) ? body.healthTools : [],
...
export async function adminGetHealthToolById(token, id) {
  try {
    const { data: body } = await api.get(`${base()}/${encodeURIComponent(id)}`, { headers: authHeader(token) });
    return body.healthTool ?? null;
  } catch (error) {
    normalizeApiError(error);
  }
}

export async function adminCreateHealthTool(token, fields, file) {
...
export async function adminUpdateHealthTool(token, id, fields, file) {
  if (file instanceof File) {
    const fd = new FormData();
...
export async function adminDeleteHealthTool(token, id) {
  try {
