...
} from "../../api/adminPrakrutiThingsToAvoid.js";
...
import { formatDate, TITLE_PREVIEW_LEN, LIST_LIMIT, LIST_SEARCH_MAX_LEN, SORT_ORDER_MIN, SORT_ORDER_MAX, sanitizeSortOrder, validateSortOrder } from "./PrakrutiThingToAvoidShared.js";
import { blockPhoneNonDigitKeyDown } from "../../../utils/personFieldValidation.js";

export function PrakrutiThingToAvoidList() {
  const dispatch = useDispatch();
  const navigate = useNavigate();
  const adminToken = useSelector((s) => s.auth.adminToken);
  const { canEdit, canDelete } = useResourcePermissions("prakruti-things-to-avoid");
  ...
  const loadRows = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const { thingsToAvoid, pagination } = await adminListPrakrutiThingsToAvoid(adminToken, {
        page,
        limit: LIST_LIMIT,
        ...(debouncedSearch ? { search: debouncedSearch } : {}),
        ...(listStatus ? { status: listStatus } : {}),
      });
      setRows(thingsToAvoid);
      setOrderEdits({});
  ...
  const onSaveOrder = async (row) => {
    ...
    try {
      await adminUpdatePrakrutiThingToAvoid(adminToken, row._id, { sortOrder: nextOrder });
      await loadRows();
    } catch (e) {
      if (e?.status === 401) return dispatch(logout());
  ...
  const onDelete = async (row) => {
    ...
    try {
      await adminDeletePrakrutiThingToAvoid(adminToken, row._id);
  ...
  const onToggleStatus = async (row) => {
    ...
    try {
      await adminUpdatePrakrutiThingToAvoid(adminToken, row._id, { status: nextStatus });
  ...
  return (
    <div className="user-page">
      <div className="page-card">
        <AdminListHeader
          title="Prakruti Things to Avoid"
          subtitle={subtitle}
          actions={
            canEdit ? (
            <button type="button" className="btn btn--primary" onClick={() => navigate("/admin/prakruti-things-to-avoid/new")}>
              Add item
            </button>
          ) : null
          }
...
<button
                    ...
                    <td>
                      <div className="row-actions">
                        <Link to={`/admin/prakruti-things-to-avoid/${row._id}`} className="icon-btn icon-btn--view" title="View">
...
<button
                          ...
                          title="Edit"
                          onClick={() => navigate(`/admin/prakruti-things-to-avoid/${row._id}/edit`)}
                        >
