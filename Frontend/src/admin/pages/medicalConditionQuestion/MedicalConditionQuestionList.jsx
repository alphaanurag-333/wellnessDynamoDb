...
export function MedicalConditionQuestionList() {
  const dispatch = useDispatch();
  const navigate = useNavigate();
  const adminToken = useSelector((s) => s.auth.adminToken);
  const { canEdit, canDelete } = useResourcePermissions("medical-condition-questions");
  ...
  const loadRows = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const { questions, pagination } = await adminListMedicalConditionQuestions(adminToken, {
        page,
        limit: LIST_LIMIT,
  ...
  const onDelete = async (row) => {
    const { isConfirmed } = await Swal.fire({
      ...
      text: "This will delete this onboarding question.",
    ...
    try {
      await adminDeleteMedicalConditionQuestion(adminToken, row._id);
  ...
  const onToggleStatus = async (row) => {
    ...
    try {
      await adminUpdateMedicalConditionQuestion(adminToken, row._id, { status: nextStatus });
    ...
    <div className="user-page">
      <div className="page-card">
        <AdminListHeader
          ...
          actions={
            canEdit ? (
            <button type="button" className="btn btn--primary" onClick={() => navigate("/admin/medical-condition-questions/new")}>
              Add question
            </button>
          ) : null
          }
...
<button
                    ...
                    <td>
                      <div className="row-actions">
                        <Link to={`/admin/medical-condition-questions/${row._id}`} className="icon-btn icon-btn--view" title="View">
...
<button
                          ...
                          title="Edit"
                          onClick={() => navigate(`/admin/medical-condition-questions/${row._id}/edit`)}
                        >
