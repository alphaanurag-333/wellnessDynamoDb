...
import { adminCreatePrakrutiQuestion, adminUpdatePrakrutiQuestion } from "../../api/adminPrakrutiQuestions.js";
...
export function PrakrutiQuestionForm({ mode = "create", initialQuestion = null }) {
  ...
  const onSubmit = async (e) => {
    ...
    try {
      if (editId) {
        await adminUpdatePrakrutiQuestion(adminToken, editId, payload);
        await Swal.fire({ icon: "success", title: "Question updated", timer: 1500 });
      } else {
        await adminCreatePrakrutiQuestion(adminToken, payload);
  ...
  return (
    <form onSubmit={onSubmit}>
      ...
      <div className="user-form__actions">
        {isEditMode ? (
          <button type="button" className="btn btn--ghost" onClick={() => navigate("/admin/prakruti-questions")}>
            Cancel edit
          </button>
        ) : (
...
export function PrakrutiQuestionAdd() {
  return (
    <div className="user-page">
      <AdminPageHeader
        title="Create Prakruti question"
        subtitle="Add a question to the Prakruti assessment."
        backTo="/admin/prakruti-questions"
      />
      <div className="page-card">
        <PrakrutiQuestionForm mode="create" />
      </div>
    </div>
  );
}
