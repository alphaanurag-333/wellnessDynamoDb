...
} from "../../api/adminPrakrutiQuestions.js";
...
export function PrakrutiQuestionList() {
  const dispatch = useDispatch();
  const navigate = useNavigate();
  const adminToken = useSelector((s) => s.auth.adminToken);
  const { canEdit, canDelete } = useResourcePermissions("prakruti-questions");
  ...
  const loadRows = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const { questions, pagination } = await adminListPrakrutiQuestions(adminToken, {
        page,
        limit: LIST_LIMIT,
  ...
  const onSaveOrder = async (row) => {
    ...
    try {
      await adminUpdatePrakrutiQuestion(adminToken, row._id, { sortOrder: nextOrder });
      await loadRows();
    } catch (e) {
      if (e?.status === 401) return dispatch(logout());
  ...
  const onDelete = async (row) => {
    const { isConfirmed } = await Swal.fire({
      ...
      text: "This will delete this Prakruti assessment question.",
    ...
    try {
      await adminDeletePrakrutiQuestion(adminToken, row._id);
  ...
  const onToggleStatus = async (row) => {
    ...
    try {
      await adminUpdatePrakrutiQuestion(adminToken, row._id, { status: nextStatus });
  ...
  return (
    <div className="user-page">
      <div className="page-card">
        <AdminListHeader
          title="Prakruti Questions"
          subtitle={subtitle}
          actions={
            canEdit ? (
            <button type="button" className="btn btn--primary" onClick={() => navigate("/admin/prakruti-questions/new")}>
              Add question
            </button>
          ) : null
          }
...
<button
                    ...
                    <td>
                      <div className="row-actions">
                        <Link to={`/admin/prakruti-questions/${row._id}`} className="icon-btn icon-btn--view" title="View">
...
<button
                          ...
                          title="Edit"
                          onClick={() => navigate(`/admin/prakruti-questions/${row._id}/edit`)}
                        >
