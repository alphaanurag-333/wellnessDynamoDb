...
export function PhysicalExerciseList() {
  ...
  const { canEdit, canDelete } = useResourcePermissions("physical-exercises");
  ...
  const loadRows = useCallback(async () => {
    ...
    try {
      ...
      setRows(physicalExercises);
      setPages(pagination?.pages ?? 1);
      setTotal(pagination?.total ?? 0);
    } catch (e) {
      if (e?.status === 401) return dispatch(logout());
      await Swal.fire({ icon: "error", title: "Load failed", text: e.message || "Failed to load physical exercises." });
    } finally {
      setLoading(false);
    }
  ...
  const onDelete = async (row) => {
    const { isConfirmed } = await Swal.fire({
      icon: "warning",
      title: "Delete physical exercise?",
    ...
    try {
      await adminDeletePhysicalExercise(adminToken, row._id);
  ...
  const onToggleStatus = async (row) => {
    ...
    try {
      await adminUpdatePhysicalExercise(adminToken, row._id, { status: nextStatus });
  ...
  const subtitle = listCountSubtitle(loading, total, "physical exercise", "physical exercises");
    ...
    <div className="user-page">
      <div className="page-card">
        <AdminListHeader
          ...
          actions={
            canEdit ? (
            <button type="button" className="btn btn--primary" onClick={() => navigate("/admin/physical-exercises/new")}>
        ...
        <div className="table-scroll">
          <table className="data-table">
            ...
            <tbody>
              {loading ? (
                <AdminTableLoaderRow colSpan={8} label="Loading physical exercises..." />
              ) : rows.length === 0 ? (
                <tr>
                  <td colSpan={8}>No physical exercises found.</td>
                </tr>
              ) : (
                rows.map((row, idx) => (
                  <tr key={row._id}>
                    ...
                    <td>
                      {row.link ? (
                        row.type === "video" ? (
                          ...
                          <a href={row.link} target="_blank" rel="noreferrer" title={row.link}>
                            Open link
                          </a>
                        )
                      ) : (
...
<button
                    ...
                    <td>
                      <div className="row-actions">
                        <Link to={`/admin/physical-exercises/${row._id}`} className="icon-btn icon-btn--view" title="View">
...
<button
                          ...
                          title="Edit"
                          onClick={() => navigate(`/admin/physical-exercises/${row._id}/edit`)}
                        >
