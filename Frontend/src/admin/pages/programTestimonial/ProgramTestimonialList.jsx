...
export function ProgramTestimonialList() {
  const dispatch = useDispatch();
  const navigate = useNavigate();
  const adminToken = useSelector((s) => s.auth.adminToken);
  const { canEdit, canDelete } = useResourcePermissions("program-testimonials");
  ...
  const loadRows = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const { programTestimonials, pagination } = await adminListProgramTestimonials(adminToken, {
        page,
        limit: LIST_LIMIT,
      ...
      setRows(programTestimonials);
      setPages(pagination?.pages ?? 1);
      setTotal(pagination?.total ?? 0);
    } catch (e) {
      if (e?.status === 401) return dispatch(logout());
      await Swal.fire({ icon: "error", title: "Load failed", text: e.message || "Failed to load program testimonials." });
    } finally {
      setLoading(false);
    }
  ...
  const onDelete = async (row) => {
    ...
    try {
      await adminDeleteProgramTestimonial(adminToken, row._id);
  ...
  const onToggleStatus = async (row) => {
    ...
    try {
      await adminUpdateProgramTestimonial(adminToken, row._id, { status: nextStatus });
  ...
  const subtitle = listCountSubtitle(loading, total, "program testimonial", "program testimonials");
    ...
    <div className="user-page">
      <div className="page-card">
        <AdminListHeader
          ...
          actions={
            canEdit ? (
            <button type="button" className="btn btn--primary" onClick={() => navigate("/admin/program-testimonials/new")}>
              Add program testimonial
            </button>
          ) : null
          }
        />
          ...
          <table className="data-table">
            ...
            <tbody>
              {loading ? (
                ...
                <tr>
                  <td colSpan={7}>No program testimonials found.</td>
                </tr>
              ) : (
                rows.map((row, idx) => (
                  <tr key={row._id}>
...
<button
                    ...
                    <td>
                      <div className="row-actions">
                        <Link to={`/admin/program-testimonials/${row._id}`} className="icon-btn icon-btn--view" title="View">
...
<button
                          ...
                          title="Edit"
                          onClick={() => navigate(`/admin/program-testimonials/${row._id}/edit`)}
                        >
