...
export function YogaList() {
  ...
  const onDelete = async (row) => {
    ...
    try {
      await adminDeleteYoga(adminToken, row._id);
  ...
  const onToggleStatus = async (row) => {
    ...
    try {
      await adminUpdateYoga(adminToken, row._id, { status: nextStatus });
  ...
  const subtitle = listCountSubtitle(loading, total, "yoga", "yoga");
      ...
      <div className="page-card">
        ...
        <div className="table-scroll">
          <table className="data-table">
            ...
            <tbody>
              {loading ? (
                <AdminTableLoaderRow colSpan={9} label="Loading yoga..." />
              ) : rows.length === 0 ? (
                <tr>
                  <td colSpan={9}>No yoga found.</td>
                </tr>
              ) : (
                rows.map((row, idx) => (
                  <tr key={row._id}>
                    ...
                    <td>
                      <div className="row-actions">
                        ...
                        <button
                          ...
                          title="Edit"
                          onClick={() => navigate(`/admin/yoga/${row._id}/edit`)}
                        >
