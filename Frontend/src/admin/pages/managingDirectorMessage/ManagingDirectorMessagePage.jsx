...
import {
  adminCreateManagingDirectorMessage,
  adminGetManagingDirectorMessage,
  adminUpdateManagingDirectorMessage,
} from "../../api/managingDirectorMessageController.js";
...
import {
  ...
  emptyForm,
  messageFromApi,
} from "./ManagingDirectorMessageShared.js";

export function ManagingDirectorMessagePage() {
  ...
  const loadRecord = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const body = await adminGetManagingDirectorMessage(adminToken);
      ...
      await Swal.fire({
        icon: "error",
        title: "Load failed",
        text: e.message || "Could not load managing director message.",
      });
    } finally {
      setLoading(false);
    }
  ...
  const onSubmit = async (e) => {
    ...
    const payload = {
      ...
      designation: String(form.designation || "Managing Director")
        .replace(/\s+/g, " ")
        .slice(0, DESIGNATION_MAX_LEN)
        .trim(),
    ...
    try {
      if (exists) {
        await adminUpdateManagingDirectorMessage(adminToken, payload);
        await Swal.fire({ icon: "success", title: "Managing director message updated", timer: 1500 });
      } else {
        await adminCreateManagingDirectorMessage(adminToken, payload);
        await Swal.fire({ icon: "success", title: "Managing director message created", timer: 1500 });
      }
      await loadRecord();
    } catch (err) {
  ...
  if (loading) {
    return <AdminPageLoader label="Loading managing director message…" />;
  }

  return (
    <div className="user-page">
      <div className="page-card">
        <AdminListHeader title="Managing director message" />
