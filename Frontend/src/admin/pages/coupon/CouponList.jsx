...
export function CouponList() {
  ...
  const loadRows = useCallback(async () => {
    ...
    try {
      ...
      await Swal.fire({ icon: "error", title: "Load failed", text: e.message || "Failed to load coupons." });
    } finally {
      setLoading(false);
    }
  ...
  const onDelete = async (row) => {
    ...
    if (!isConfirmed || !adminToken || !couponId) return;
    try {
      await adminDeleteCoupon(adminToken, couponId);
      await Swal.fire({ icon: "success", title: "Coupon deleted", timer: 1500 });
      await loadRows();
    } catch (e) {
      if (e?.status === 401) return dispatch(logout());
      await Swal.fire({ icon: "error", title: "Delete failed", text: e.message || "Could not delete coupon." });
    }
  };
  ...
  const onToggleStatus = async (row) => {
    ...
    try {
      await adminUpdateCoupon(adminToken, couponId, { status: nextStatus });
      await Swal.fire({
        icon: "success",
      ...
      <div className="page-card">
        ...
        <div className="table-scroll">
          <table className="data-table">
            ...
            <tbody>
              {loading ? (
                <AdminTableLoaderRow colSpan={7} label="Loading coupons..." />
              ) : rows.length === 0 ? (
                <tr>
                ...
                rows.map((row, idx) => (
                  <tr key={getCouponId(row) || idx}>
                    <td className="data-table__muted">{(page - 1) * LIST_LIMIT + idx + 1}</td>
                    <td title={row.title || ""}>{truncateText(row.title, TITLE_MAX_LEN)}</td>
                    <td title={row.couponCode || ""}>{truncateText(row.couponCode, COUPON_CODE_MAX_LEN)}</td>
                    ...
                    <td>
                      <div className="row-actions">
                        ...
                        <button
                          ...
                          title="Edit"
                          onClick={() => navigate(`/admin/coupons/${getCouponId(row)}/edit`)}
                        >
