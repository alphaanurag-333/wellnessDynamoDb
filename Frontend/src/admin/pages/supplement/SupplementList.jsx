...
export function SupplementList() {
  ...
  const { canEdit, canDelete } = useResourcePermissions("supplements");
  ...
  const loadRows = useCallback(async () => {
    ...
    try {
      ...
      await Swal.fire({ icon: "error", title: "Load failed", text: e.message || "Failed to load supplements." });
    } finally {
      setLoading(false);
    }
  ...
  const onDelete = async (row) => {
    ...
    try {
      await adminDeleteSupplement(adminToken, row._id);
  ...
  const onToggleStatus = async (row) => {
    ...
    try {
      await adminUpdateSupplement(adminToken, row._id, { status: nextStatus });
  ...
  const subtitle = listCountSubtitle(loading, total, "supplement", "supplements");
    ...
    <div className="user-page">
      <div className="page-card">
        <AdminListHeader
          ...
          actions={
            canEdit ? (
            <button type="button" className="btn btn--primary" onClick={() => navigate("/admin/supplements/new")}>
              Add supplement
            </button>
          ) : null
          }
        ...
        <div className="table-scroll">
          <table className="data-table">
            ...
            <tbody>
              {loading ? (
                <AdminTableLoaderRow colSpan={8} label="Loading supplements..." />
              ) : rows.length === 0 ? (
                <tr>
                ...
                rows.map((row, idx) => (
                  <tr key={row._id}>
                    ...
                    <td>
                      {row.image ? (
                        <AdminMediaImage path={row.image} width={44} height={44} radius={8} alt="" />
                      ) : (
                        "—"
                      )}
                    </td>
                    ...
                    <td className="data-table__muted">
                      {row.packSize != null ? `${row.packSize} ${row.unit || ""}`.trim() : "—"}
                    </td>
                    <td className="data-table__muted">{formatPrice(row.price)}</td>
...
<button
                    ...
                    <td>
                      <div className="row-actions">
                        <Link to={`/admin/supplements/${row._id}`} className="icon-btn icon-btn--view" title="View">
...
<button
                          ...
                          title="Edit"
                          onClick={() => navigate(`/admin/supplements/${row._id}/edit`)}
                        >
