...
export function HealthDisorderList() {
  ...
  const { canEdit, canDelete } = useResourcePermissions("health-disorders");
  ...
  const loadRows = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const { healthDisorders, pagination } = await adminListHealthDisorders(adminToken, {
        page,
        limit: LIST_LIMIT,
      ...
      setRows(healthDisorders);
      setPages(pagination?.pages ?? 1);
      setTotal(pagination?.total ?? 0);
    } catch (e) {
      if (e?.status === 401) return dispatch(logout());
      await Swal.fire({ icon: "error", title: "Load failed", text: e.message || "Failed to load health disorders." });
    } finally {
      setLoading(false);
    }
  ...
  const onDelete = async (row) => {
    ...
    try {
      await adminDeleteHealthDisorder(adminToken, row._id);
  ...
  const onToggleStatus = async (row) => {
    ...
    try {
      await adminUpdateHealthDisorder(adminToken, row._id, { status: nextStatus });
  ...
  const subtitle = listCountSubtitle(loading, total, "health disorder", "health disorders");
    ...
    <div className="user-page">
      <div className="page-card">
        <AdminListHeader
          ...
          actions={
            canEdit ? (
            <button type="button" className="btn btn--primary" onClick={() => navigate("/admin/health-disorders/new")}>
              Add health disorder
            </button>
          ) : null
          }
        ...
        <div className="table-scroll">
          <table className="data-table">
            ...
            <tbody>
              {loading ? (
                <AdminTableLoaderRow colSpan={7} label="Loading health disorders..." />
              ) : rows.length === 0 ? (
                <tr>
                  <td colSpan={7}>No health disorders found.</td>
                </tr>
              ) : (
                rows.map((row, idx) => (
                  <tr key={row._id}>
...
<button
                    ...
                    <td>
                      <div className="row-actions">
                        <Link to={`/admin/health-disorders/${row._id}`} className="icon-btn icon-btn--view" title="View">
...
<button
                          ...
                          title="Edit"
                          onClick={() => navigate(`/admin/health-disorders/${row._id}/edit`)}
                        >
