...
function healUsersBase(userId) {
  return `/assistant/heal-users/${userId}/physical-exercises`;
}

export async function assistantListUserPhysicalExercises(token, userId) {
  try {
...
export async function assistantAssignPhysicalExercises(token, userId, { exerciseIds }) {
  try {
...
export async function assistantRemovePhysicalExercise(token, userId, assignmentId) {
  try {
